...
function plot_simulated_mean_queue_length(parameters::NetworkParameters, scenario_number::Int)
    ...
    for (i, max_time) in enumerate(max_times)
        ...
        for (i, rho_star) in collect(enumerate(rho_stars)) 
            ...
            function record_integral(state::NetworkStateCustomers, time::Float64)
                (time >= warm_up_time) && (riemann_sum += prev_total * (time - prev_time))
                prev_total = sum(map(length, state.queues))
                prev_time = time
            end

            sim_net_customers(new_parameters, max_time = max_time, warm_up_time = warm_up_time,
...
for (i, scenario) in enumerate(scenarios)
    ...
    savefig(p1,"_img/task_3_test_1_scenario_$(i)_simulated.png") 
    savefig(p2,"_img/task_3_test_1_scenario_$(i)_error.png") 
