...
function compare_arrival_rates(parameters::NetworkParameters, scenario_number::Int)
    ...
    for (i,c_s) in enumerate(c_s_values)
        ...
        println("The simulated arrival rates are: ", simulated_arrival_rates[1:max_index])
        println("The theoretical arrival rates are: ", theoretical_arrival_rates[1:max_index])
